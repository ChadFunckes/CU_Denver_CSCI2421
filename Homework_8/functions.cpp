...
#include "grocery.h"  // Provides bool_source, averager, washer definitions
using namespace std;
...
namespace main_savitch_8A
{
	...
	void washer::set_time(unsigned int s)
	{
		seconds_for_wash = s;
		wash_time_left = 0;
	}

	unsigned int washer::get_time(){
		return seconds_for_wash;
	}
